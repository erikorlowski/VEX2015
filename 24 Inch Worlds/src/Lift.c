...
int liftToHeight(Lift lift, int heightSP, int deadBand)
{
	...
	int integralContribution = (error > 0) ? 0 : integralOffset;

	int proportionalContribution = (int) (kP * error);

	if(inDeadBand(error, 0, deadBand)) liftAtSpeed(lift, 20);
	else liftAtSpeed(lift, proportionalContribution + integralContribution);
...
{
	return ((getPot(lift.leftPot) + getPot(lift.rightPot))/2);
}
